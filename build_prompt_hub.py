    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-wrap items-center justify-between gap-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-[11px] text-slate-400 hidden sm:block">คลังโปรมป์ต์ 200 ข้อโหมด AI หาเงิน + 100 ข้อโหมดวิศวกรรมเว็บ (รวม 300 ข้อ)</p>
        </div>
      </div>
...
`;
    ...
    async function runAiGeneration() {{
      ...
      document.getElementById('btn-run-ai-text').textContent = 'กำลังสร้างผลลัพธ์...';
      outBox.innerHTML = '<span class="text-emerald-400 animate-pulse">กำลังประมวลผลคำสั่งกับโมเดล Gemini...</span>';
      ...
      try {{
        ...
        if (res.ok) {{
          ...
          outBox.textContent = '❌ Error: ' + (data.error || 'Failed to generate output');
        }}
      }} catch (err) {{
        outBox.textContent = '❌ Network Error: ตรวจสอบว่าเซิร์ฟเวอร์ server.py รันอยู่ปกติ';
      }} finally {{
        btn.disabled = false;
        document.getElementById('btn-run-ai-icon').textContent = '⚡';
